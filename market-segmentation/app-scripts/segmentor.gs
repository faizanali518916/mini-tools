function generateSegmentationReports() {
	...
	try {
		const startColLetter = toolSheet.getRange('E6').getValue().toString().toUpperCase();
		const endColLetter = toolSheet.getRange('E7').getValue().toString().toUpperCase();
		...
		for (let col = startCol; col <= endCol; col++) {
			const colName = dataSheet.getRange(2, col).getValue() || 'Value';
			const numRowsToRead = lastRow - 2;
			if (numRowsToRead <= 0) continue;

			const dataValues = dataSheet.getRange(3, col, numRowsToRead).getValues();
			const salesValues = dataSheet.getRange(3, salesCol, numRowsToRead).getValues();
			const revenueValues = dataSheet.getRange(3, revenueCol, numRowsToRead).getValues();
			...
			addMarketShareChart(reportSheet, cursorRow, startDataCol, result.numRows, colName);
		...
		logRange.setValue(`✅ Success! Dashboard generated at ${new Date().toLocaleTimeString()}`);
	} catch (e) {
		logRange.setValue('❌ Error: ' + e.message);
	}
}

function renderFormattedTable(sheet, startRow, startCol, title, data, totalSales, totalRev, sortedKeys) {
	...
	headerRange
		.setValues([[title, 'MONTHLY SALES', 'MONTHLY REVENUE', 'MARKET SHARE UNIT', 'MARKET SHARE REVENUE']])
	...
	dataRange.setBorder(null, true, null, true, null, true, 'black', SpreadsheetApp.BorderStyle.DOTTED);
	sheet
	...
	sheet.getRange(startRow + 2, startCol + 1, numRows + 1, 1).setNumberFormat('#,##0');
	sheet.getRange(startRow + 2, startCol + 2, numRows + 1, 1).setNumberFormat('$#,##0.00');
	sheet.getRange(startRow + 2, startCol + 3, numRows, 2).setNumberFormat('0.00%');
...
function addMarketShareChart(sheet, startRow, tableStartCol, numRows, title) {
	...
	const unitShareRange = sheet.getRange(startRow + 2, tableStartCol + 3, numRows, 1);
	const revShareRange = sheet.getRange(startRow + 2, tableStartCol + 4, numRows, 1);
